...
. Push the cities-node-ui +
Before pushing the app, check if the route exists and we can reach it.
+
[source,bash]
----
$ cd cities-node-ui
$ cf check-route <first-initial><last-initial>-cities-node-ui cfapps.io
$ cf push <first-initial><last-initial>-cities-node-ui -i 1 -m 512M -p build/libs/cities-service-0.0.1-SNAPSHOT.jar
----

* Be sure to name your application '<first-initial><last-initial>-cities-node-ui'

. Open the app url http://<first-initial><last-initial>-cities-node-ui.cfapps.io from a Web Browser:

image:./images/cities-node-ui.png[DevOps on CF]
...
=== Discussion: Part 2
In this part of the workshop we created a database service from the marketplace, push a app, bound it to the database service, monitored the health of the app and scale the app.
